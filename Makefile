...
# adjust this setting according to your bank configuration:
BANKOPTS :=  -Wl-b_BANK1=0x14000
# any options to SDCC compiler, e.g. optimization settings
SDCCOPTS := --opt-code-speed
...
$(TARGETDIR)%.ihx: $(OBJECTS)
	sdcc -L$(SMSLIB_DIR) -o$@ -mz80 --no-std-crt0 --data-loc 0xC000 $(BANKOPTS) $(SMSLIB_DIR)/crt0_sms.rel $(MAINS) $(filter-out $(MAINS),$(OBJECTS)) SMSlib.lib $(PSGLIB) || rm $@

# compile stage - generally runs once per .c file found in the source folder
$(TARGETDIR)%.rel: $(SOURCEDIR)%.$(SOURCEEXT) $(ASSETSHEADER)
	sdcc -I$(SMSINC_DIR) $(SDCCOPTS) -c -mz80 -o$(TARGETDIR) --peep-file $(SMSLIB_DIR)/peep-rules.txt $(PSGMACRO) $<
